...
type MatcherParam<M> = M extends (param: string) => param is infer U
  ...
  : string;
type RouteParams = {};
type RouteId = '/auth/forgot-password';
...
type ExcludeActionFailure<T> =
  T extends Kit.ActionFailure<any> ? never : T extends void ? never : T;
...
type ExtractActionFailure<T> =
  T extends Kit.ActionFailure<infer X> ? (X extends void ? never : X) : never;
type ActionsFailure<T extends Record<string, (...args: any) => any>> = {
  [Key in keyof T]: Exclude<
    ExtractActionFailure<Awaited<ReturnType<T[Key]>>>,
    void
  >;
}[keyof T];
type ActionsExport = typeof import('./proxy+page.server.js').actions;
export type SubmitFunction = Kit.SubmitFunction<
  Expand<ActionsSuccess<ActionsExport>>,
  Expand<ActionsFailure<ActionsExport>>
>;
